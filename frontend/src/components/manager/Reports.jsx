...
import { BarChart3, Download, Calendar, TrendingUp, Users, Package, DollarSign, PieChart } from 'lucide-react';
...
const Reports = () => {
  ...
  /**
   * Calculate percentage change (mock function for demo)
   */
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600">Analyze sales performance and generate reports</p>
        </div>
                ...
                <div key={item.meal_id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500">
                      {((item.total_revenue / (reports.total_revenue || 1)) * 100).toFixed(1)}% of revenue
                    </p>
                  </div>
                </div>
              ))}
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {reports.daily_sales.map(day => {
                const percentage = (day.revenue / (reports.max_daily_revenue || 1)) * 100;
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {isLoading ? (
          ...
          ) : reports.order_status_distribution ? (
            <div className="space-y-3">
              {Object.entries(reports.order_status_distribution).map(([status, count]) => (
                <div key={status} className="flex items-center justify-between">
                  <span className="text-sm text-gray-600 capitalize">{status.replace('_', ' ')}</span>
                  <div className="flex items-center space-x-3">
                    <span className="text-sm font-medium text-gray-900">{count}</span>
                    <span className="text-xs text-gray-500">
                      ({((count / (reports.total_orders || 1)) * 100).toFixed(1)}%)
                    </span>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center py-8">
              <Package className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-600">No order distribution data available</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          ) : reports.revenue_by_category && reports.revenue_by_category.length > 0 ? (
            <div className="space-y-3">
              {reports.revenue_by_category.map(category => (
                <div key={category.category_id} className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">{category.category_name}</span>
                  <div className="flex items-center space-x-3">
                    <span className="text-sm font-medium text-gray-900">
                      MK {parseFloat(category.total_revenue).toFixed(2)}
                    </span>
                    <span className="text-xs text-gray-500">
                      ({((category.total_revenue / (reports.total_revenue || 1)) * 100).toFixed(1)}%)
                    </span>
                  </div>
                </div>
              ))}
